    ...
    : QWidget(parent)
{
    ...
    connect(m_timer, &QTimer::timeout, this, &Dashboard::updateCharts);
}

Dashboard::~Dashboard() {}

void Dashboard::setServer(WebDavServer *server) { m_server = server; }

void Dashboard::startUpdates()
{
    if (!m_server) return;
    m_lastBytesSent     = m_server->bytesSent();
    m_lastBytesReceived = m_server->bytesReceived();

    // Сохраняем начальные значения времени процесса и wall‑time
#ifdef Q_OS_WIN
...
void Dashboard::stopUpdates() { m_timer->stop(); }
...
void Dashboard::setupUi()
{
    ...
    // ── CPU chart (одна линия – загрузка процесса) ────────────────────────
    ...
    m_cpuSeries->setColor(QColor(232, 17, 35));   // красный #E81123
...
// ---------------------------------------------------------------------------
double Dashboard::getCpuUsagePercent()
{
#ifdef Q_OS_WIN
    FILETIME create, exit, kernel, user;
    if (!GetProcessTimes(GetCurrentProcess(), &create, &exit, &kernel, &user))
    ...
    // Процент от одного ядра
    double percentSingleCore = 100.0 * procDelta / wallDelta;
    // Переводим в процент от суммарной мощности всех ядер (как в диспетчере задач)
...
// ---------------------------------------------------------------------------
qint64 Dashboard::getProcessMemoryMB()
{
#ifdef Q_OS_WIN
